...
class OrexImageMergingV2:
    # Словари вынесены на уровень класса, чтобы не пересоздаваться при каждом вызове функции
    COLOR_MAP = {
    ...
    def INPUT_TYPES(s):
        ...
        for i in range(1, 21): 
            ...
            optional_inputs[f"caption{i}"] = ("STRING", {"default": ""}) 
    ...
    def merge_images_v2(self, image_number, merging_mode, main_resolution_image,
                     footer_height, font_name, font_size, mode, border_thickness,
                     upscale_method, megapixels, **kwargs):

        bg_color, fg_color = self.COLOR_MAP.get(mode, ("#FFFFFF", "#000000"))
        ...
        for i in range(1, image_number + 1):
            ...
            if img_tensor is not None:
                ...
                cap = kwargs.get(f"caption{i}", "")
                texts_list.append(cap)
        ...
        def get_layout(b_w, b_h):
            ...
            # Корректировка для специальных сеток
        ...
        resampler = self.RESAMPLE_MAP.get(upscale_method, Image.Resampling.LANCZOS)
        resized_images = []
        for i, img in enumerate(valid_images):
