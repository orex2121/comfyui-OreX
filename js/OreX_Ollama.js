...
// Массив описаний параметров для узла Ollama (OreX)
const OLLAMA_HELP_DESCRIPTIONS = [
    ...
    { name: "system_preset", label: "System Preset / Пресет системы", desc: "Pre-configured system prompt presets", ru_desc: "Предустановленные готовые системные промпты" },
    { name: "model_key", label: "Model Key / Выбор модели", desc: "Select the specific Ollama model to use", ru_desc: "Выбор конкретной установленной модели Ollama" },
    { name: "include_reasoning", label: "Include Reasoning / Мышление модели", desc: "Enable or disable deep thinking process (for R1 / reasoning models)", ru_desc: "Включение или отключение вывода цепочки рассуждений (скрывает/показывает теги <think>)" },
    { name: "auto_unload", label: "Auto Unload / Автовыгрузка", desc: "Automatically unload model from VRAM after generation", ru_desc: "Автоматически выгружать модель из видеопамяти после работы" },
    { name: "unload_delay", label: "Unload Delay / Задержка выгрузки", desc: "Time in seconds to wait before unloading the model", ru_desc: "Время в секундах перед выгрузкой модели" },
    { name: "seed", label: "Seed / Сид", desc: "Randomness control for generations", ru_desc: "Контроль случайности для воспроизводимости" },
    ...
    { name: "generation_parameters", label: "Gen Parameters / Доп. параметры", desc: "Enable or disable sampling configuration below", ru_desc: "Включить или выключить настройки семплирования ниже" },
    { name: "temperature", label: "Temperature / Температура", desc: "Creativity slider. Higher values mean more random output", ru_desc: "Ползунок креативности. Выше значение — более случайный ответ" },
    ...
    { name: "top_p", label: "Top P", desc: "Nucleus sampling threshold based on cumulative probability", ru_desc: "Порог выборки по кумулятивной вероятности токенов" },
    { name: "repeat_penalty", label: "Repeat Penalty / Штраф за повторы", desc: "Prevents the model from repeating the same phrases", ru_desc: "Предотвращает зацикливание и повторение одинаковых фраз" }
];

app.registerExtension({
    name: "OreXOllama.Antigravity.Unique",
    async beforeRegisterNodeDef(nodeType, nodeData) {
        // Проверяем соответствие имени системного маппинга для узла Ollama
        if (nodeData.name !== "orex Ollama") return;
        ...
        proto._drawHelpSidebar = function (ctx) {
            ...
            OLLAMA_HELP_DESCRIPTIONS.forEach((item) => { maxLabelW = Math.max(maxLabelW, ctx.measureText(item.label).width); ctx.font = descFont; maxDescW = Math.max(maxDescW, ctx.measureText(`- ${item.desc} / ${item.ru_desc}`).width); ctx.font = labelFont; });
            ...
            let fallbackY = by + 60;
            OLLAMA_HELP_DESCRIPTIONS.forEach((item) => {
