...
const LMSTUDIO_HELP_DESCRIPTIONS = [
    ...
    { icon: "🧠", name: "show_reasoning", label: "Show Thinking / Показать рассуждения", desc: "🟢ON - display the reasoning chain (<think> tags) in the output; 🔴OFF - hide reasoning, show only the final answer", ru_desc: "🟢ON - показать цепочку рассуждений (теги <think>) в ответе; 🔴OFF - скрыть рассуждения, показать только финальный ответ" },
    { icon: "⚡", name: "enable_thinking", label: "Thinking ON / Включить рассуждения", desc: "🟢ON - model thinks as usual with full reasoning chain; 🔴OFF - disable thinking (thinking_budget=0), model responds immediately without reasoning tokens", ru_desc: "🟢ON - модель думает как обычно с полной цепочкой рассуждений; 🔴OFF - отключить thinking (thinking_budget=0), модель отвечает сразу без reasoning-токенов" },
